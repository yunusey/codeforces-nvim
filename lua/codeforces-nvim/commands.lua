...
M.compile = function(compiler, exit_function)
	...
	vim.fn.jobstart(compiler, {
		...
		on_exit = function(_, _)
			if errors then
				codeforces.options.notify(
					"Compilation Failed ✗",
					table.concat(all_data, "\n"),
					"error"
				)
			else
				codeforces.options.notify(
...
--- @param errors <string, string[], string[]>[]
...
M.run_normally = function(problem)
	local command = '"' .. get_terminal_run_command(problem) .. '"'
...
M.save_custom_test = function(problem, buf, win)
	...
	local file_path = vim.fs.joinpath(codeforces.paths.solutions, codeforces.get_current_contest(),
